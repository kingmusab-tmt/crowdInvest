...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get users to seed sample investments for
    ...
    if (communityUsers.length === 0) {
      return NextResponse.json(
        { error: "No users found to seed investments for" },
        { status: 400 }
      );
    }
    ...
    // Create sample investment suggestions
    ...
    return NextResponse.json(
      {
        ...
        data: {
          investmentsCreated: createdInvestments.length,
