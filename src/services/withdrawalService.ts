...
export async function getWithdrawalRequests(): Promise<WithdrawalRequest[]> {
  ...
  const requestList = requestSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as WithdrawalRequest));
 ...
 */
export async function createWithdrawalRequest(requestData: Omit<WithdrawalRequest, 'id'>): Promise<WithdrawalRequest> {
  const requestsCollection = collection(db, 'withdrawalRequests');
 ...
 */
export async function deleteWithdrawalRequest(requestId: string): Promise<void> {
  const requestDoc = doc(db, 'withdrawalRequests', requestId);
...
async function seedInitialWithdrawalRequests(): Promise<WithdrawalRequest[]> {
    ...
    for (const requestData of initialRequests) {
        const docRef = await addDoc(collection(db, 'withdrawalRequests'), requestData);
        seededRequests.push({ id: docRef.id, ...requestData });
    }

    console.log('Database seeded with initial withdrawal requests.');
    return seededRequests.sort((a, b) => new Date(b.requestDate).getTime() - new Date(a.requestDate).getTime());
}
