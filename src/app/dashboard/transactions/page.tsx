...
export default function UserTransactionsPage() {
    ...
    const fetchTransactions = async () => {
      try {
        ...
        const userTransactions = allTransactions.filter(
          (t) => t.userEmail === LOGGED_IN_USER_EMAIL
        );
  ...
  const getTypeBadgeInfo = (
    ...
    switch (type) {
      ...
      case "Withdrawal":
        return {
          variant: "secondary",
          icon: <ArrowDownLeft className="h-3 w-3 text-red-500" />,
        };
      case "Investment":
        return { variant: "outline", icon: <Briefcase className="h-3 w-3" /> };
      case "Profit Share":
        return {
          variant: "outline",
      ...
      case "Assistance":
        return {
          variant: "outline",
          icon: <PiggyBank className="h-3 w-3 text-amber-500" />,
        };
      default:
  ...
  const getTransactionDescription = (transaction: Transaction) => {
    switch (transaction.type) {
      case "Deposit":
        return "Card deposit via Monify";
      case "Withdrawal":
        return `Request to ${transaction.userName}'s Bank`;
      case "Investment":
        return `Investment in a community project`;
      case "Profit Share":
        return `From a community investment`;
      case "Assistance":
        return "Financial assistance from community";
      default:
        return "Transaction";
    }
  };
    ...
    <div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            An overview of all your deposits, withdrawals, and investments.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
