...
async function migrate() {
  ...
  if (depositCount > 0) {
    console.log(
      `Found ${depositCount} transactions with type 'Deposit' → migrating to 'Monthly_Contribution'...`
    );
  ...
  if (withdrawalCount > 0) {
    console.log(
      `Found ${withdrawalCount} transactions with type 'Withdrawal' → migrating to 'refund_deposit'...`
    );
    await Transaction.updateMany(
  ...
  console.log(
    `Backfilled isAdminTransaction on ${backfill.modifiedCount} records`
  );
